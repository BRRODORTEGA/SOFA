...
export default function ProdutosTable<T extends { id?: string }>({ columns, rows, basePath, emptyText }: Props<T>) {
      ...
      <table className="min-w-full text-base">
        <thead>
          <tr className="bg-gradient-to-r from-gray-50 to-gray-100">
            {columns.map((col) => (
              <th key={String(col.key)} className="border-b border-gray-200 px-4 py-3 text-left text-xs font-semibold text-gray-700">
                {col.header}
              </th>
            ))}
