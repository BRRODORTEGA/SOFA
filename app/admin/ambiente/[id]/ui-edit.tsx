...
export default function EditAmbiente({ item }: { item: { id: string; nome: string; ativo: boolean } }) {
  ...
  async function onSubmit(values: { nome: string; ativo: boolean }) {
    try {
      const payload = { nome: values.nome, ativo: Boolean(values.ativo) };
      const res = await fetch(`/api/ambientes/${item.id}`, {
        method: "PUT",
  ...
  async function onDelete() {
    if (!confirm("Excluir este tipo de ambiente?")) return;
    try {
      const res = await fetch(`/api/ambientes/${item.id}`, { method: "DELETE" });
      const data = await res.json();
      if (res.ok && data.ok) {
        router.push("/admin/ambiente");
        router.refresh();
  ...
  return (
    <FormShell title="Editar Tipo de Ambiente">
      <form id="edit-ambiente-form" onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        <div>
