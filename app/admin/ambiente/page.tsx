...
export default async function Page({ searchParams }: { searchParams: { q?: string; limit?: string; offset?: string } }) {
  ...
  try {
    ...
    const [items, total] = await Promise.all([
      prisma.ambiente.findMany({ where, take: limit, skip: offset, orderBy: { createdAt: "desc" } }),
    ...
    return (
      <div>
        <h1 className="mb-6 text-3xl font-bold text-gray-900">Tipos de Ambiente</h1>
        <AdminToolbar createHref="/admin/ambiente/new" />
        <AdminTableWrapper
          columns={[
